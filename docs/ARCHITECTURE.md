...
- **No UI kit** ([ADR 0010](adr/0010-styling-approach.md)). `StyleSheet` plus local design tokens, light/dark driven by system appearance, because a Material component library would impose Android conventions on iOS. The cost is presentation components written by hand instead of imported. Debug builds add an in-memory scheme override behind a gear in the dev-only form tab, so dark mode is reviewable without leaving the app.
