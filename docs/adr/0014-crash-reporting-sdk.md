...
## Consequences
Crash and content-failure visibility now goes through one pipeline instead of `console.error`/`console.warn` alone. Source-map upload for readable native-build stack traces is not configured yet — deferred until a release pipeline needs it, and left as a manual step in the interim.
