...
- Cards get the same platform split as the tab bar's material, one step further: iOS renders a distinct raised "pill" (`borderRadius: 20`, a soft directional shadow) via the shared `Surface` component (`src/presentation/ui/Surface.tsx`); Android keeps the flatter `surfaceElevated` + `elevation: 2` convention already established for elevation elsewhere.
