...
Deliberately out of scope: Material typography (Roboto is already the Android system font), Material You dynamic colour (fights the fixed brand palette), and a `tabBarLabelStyle` for CMS-flagged `highlighted` destinations — that styling API is still wired from bootstrap through `TabNavigator`, but has nothing to attach to now that the tab bar is icon-only.
