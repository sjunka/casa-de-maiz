...
## Test file layout
Test files live under the repo-root `__tests__`, mirroring the layer tree rather than sitting beside the module they cover (`__tests__/core/contract/appVersion.test.ts`, not `src/core/contract/appVersion.test.ts`). A test's same-folder import of its module is written through the module's layer alias (`@core/contract/appVersion`) instead of a relative `./` path, since the test file no longer lives next to what it tests. This keeps every test outside `src` in one folder, alongside `App.test.tsx` for the root `App.tsx`. This is the default for new tests going forward.
