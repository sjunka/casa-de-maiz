...
iOS needs one native line Android doesn't: `AppDelegate.swift` forwards a warm-start URL to React Native's `Linking` bridge via `RCTLinkingManager.application(_:open:options:)`. Without it, `Linking.getInitialURL()` still resolves on cold start (UIKit hands the launch URL to `didFinishLaunchingWithOptions` regardless), but the `Linking` `'url'` event never fires for a link arriving while the app is already running — confirmed by testing on a real simulator, where cold start worked immediately but warm start silently did nothing until this hook was added. Android's `ReactActivity` already forwards `onNewIntent` to `Linking` internally, so `AndroidManifest.xml`'s intent filter alone is sufficient there.
