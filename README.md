...
- **Scroll-percentage alert triggers.** Only the `load` trigger with `delayMs` is implemented, since it's the one the live CMS actually uses; a scroll-based trigger needs a scroll-position seam no current screen has.
